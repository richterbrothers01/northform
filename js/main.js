...
(function () {
    ...
    function heroIntro() {
        ...
        /* Signature mark: the logo wipes into view like a doorway opening */
        var heroLogoImg = $('#heroLogoImg');
        if (heroLogoImg) {
            gsap.set(heroLogoImg, { clipPath: 'inset(0 0 100% 0)', opacity: 0 });
            tl.to(heroLogoImg, {
    ...
    /* ----------------------------------------------------------------------
       ...
       ---------------------------------------------------------------------- */
    if (!reduced) {
        var aboutLogoImg = $('.about__logo');
        if (aboutLogoImg) {
            gsap.set(aboutLogoImg, { clipPath: 'inset(0 0 100% 0)', opacity: 0 });
            gsap.to(aboutLogoImg, {
                clipPath: 'inset(0 0 0% 0)',
                opacity: 0.92,
                duration: 1.3,
                ease: 'power2.inOut',
